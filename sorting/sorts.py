...
def SortsComparison():
    ...
    for i in range(m):
        ...
        p3.join()
        p4.join()
        ABS.append([QBS.get(),l])
        ASS.append([QSS.get(),l])
        AIS.append([QIS.get(),l])
        AQS.append([QQS.get(),l])
    ...
    QQS.close()
    QQS.join_thread()
    plt.plot(ABS, color="r",label='пузырьковая сортировка')
    plt.plot(ASS, color="g",label='сортировка шейкера')
    plt.plot(AIS, color="b",label='сортировка вставками')
    plt.plot(AQS, color="y",label='быстрая сортировка')
    plt.legend()
    plt.show()
